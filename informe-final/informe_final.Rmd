...
veure.tendencia = function(x) {
  ...
  ggplot(data = simulacions.res) +
    ...
    labs(x = "mida", y = "arrel dels residus")
}
```
...
A l'assignatura de Fonaments de la computació, acabem de veure que hi ha **problemes decidibles i indecidibles**, en aquest estudi podríem definir **el problema de l'ordenació d'un vector**, que **és clarament decidible**, ja que **hi ha $n$ algoritmes que el poden resoldre**, i a més, **els algoritmes operen en temps polinòmic**. 

Entre aquests $n$ algoritmes que resolen el problema de l'ordenació d'un vector, en aquest estudi ens centrarem en només tres: **bubblesort** que és $O(n^2)$; **quicksort** que és $O(n \cdot log(n))$ en el millor i mitjà dels casos, però en el cas pitjor és $O(n^2)$ amb vectors ordenats; i **mergesort** que és $O(n \cdot log(n))$. Aquests dos últims algorismes *quicksort* i *mergesort* sabem per l'assignatura d'EDA que són algorismes de **divideix i venç**, per tant, *recursius*. En aquest estudi ens centrarem a respondre a la pregunta: **Quin algoritme és millor?**, entenent **millor** com: **Quin algoritme és més eficient?**

Durant l'estudi també sorgiran preguntes com: **Quina és la forma del temps d'execució?**, i després d'ajustar un model per cada algoritme entre el temps i la mida, sorgirà la pregunta: **Quina forma segueixen aquests errors?** Per respondre a cada pregunta farem servir conceptes de l'assignatura d'estadística.
...
I una altra que conté informació del temps que ha trigat cada simulació amb cada algoritme, amb vectors de mides i ordenacions diferents, és a dir, amb vectors ordenats i desordenats, (`simulacions`), i podem descriure cada temps d'execució fent ser `summary()`.
...
I no podem respondre el `[contrast d'homocedasticitat]`, ja que a la funció `bptest()` li pasem els models ajustats, de totes maneres si **no podem assumir homocedasticitat ni normalitat** dels residus, **els intervals de confiança no ens aporten res de nou**.
...
Finalment, observant els gràfics veiem que **no semblen seguir cap tendència**, tot i que **el test d'homocedasticitat ens dona un $valor-p < \alpha$**. Potser devem estar veient un **error tipus 1**, rebutjar $H_0$ quan és certa, en tot cas hauríem de calcular la $P(\text{rebutjar }H_0 | H_0\text{ és certa})$.
...
També és interessant que treballant amb el logaritme dels temps el test d'homoscedasticitat ens porta a rebutjar la hipòtesi nul·la, però veient els gràfics, veiem que els residus no semblen seguir cap tendència, és a dir, els residus semblen heterocedastics, sobretot els residus de quicksort i mergesort.

Hem donat una resposta a la pregunta de quina era la forma dels errors, i **donant una resposta a la pregunta inicial**, de **quin algoritme és millor?** Ara podem dir que, **el millor algoritme és el més estable**, és a dir, el **millor algoritme** és el que **els residus del seu model semblen més homocedastics**, i a més, **mantenen la forma del temps amb vectors desordenats i ordenats**. Per tot això, **el millor algoritme és mergesort**, i **el pitjor és bubblesort**, i **quicksort estaria al mig**.
